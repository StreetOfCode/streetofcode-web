...
export const QuizQuestion = ({
  ...
  useEffect(() => {
    const fetchSelectedAnswers = async () => {
      const previouslySelectedAnswers: QuizQuestionUserAnswer[] =
        await (await Api.authFetch(Api.previousAnswersByQuizIdUrl(question.quiz.id))).json()

      const savedAnswers = previouslySelectedAnswers
        .filter((a) => a.question.id === question.id)

      if (savedAnswers.length === 0) {
        setAnsweredCorrectly(null)
      } else {
        const isAnsweredCorrectly = savedAnswers.filter((x) => x.isCorrect).length !== 0
        setAnsweredCorrectly(isAnsweredCorrectly)
        onQuestionFinished(isAnsweredCorrectly)
      }

      setSelectedAnswerIds(previouslySelectedAnswers
        .filter((a) => a.question.id === question.id)
        .map((a) => a.answer)
        .flat()
        .map((a) => {
  ...
  const onSubmit = async () => {
    const response = await Api.authPost<QuizQuestionUserAnswerRequest>(Api.answerQuestionUrl(), {
  ...
  return (
    <Flex alignSelf="stretch" alignItems="stretch" gap="12px" direction="column">
        ...
        <RadioGroup
          ...
          applyColor={(i: number) => {
            const shouldBeColoredGreen =
              (selectedAnswerIds[0] === question.answers[i].id) && wasAnsweredCorrectly
            ...
            const shouldBeColoredRed =
              wasAnsweredCorrectly === false && (selectedAnswerIds[0] === question.answers[i].id)
          ...
          selected={question.answers.map((x) => x.id).indexOf(selectedAnswerIds[0])}
      ...
      <Flex alignSelf="stretch" justifyContent="space-between">
        ...
        <Text>{question.questionOrder}/{questionNumber}</Text>
      </Flex>
    </Flex>
  )
}
...
const WrappedFlex = styled(Flex)<{backgroundColor?: string, color?: string, borderColor?: string}>`
